...
class LPKUtil {
    ...
    class func showNick(uid: String?, in message: NIMMessage?) -> String? {
        guard let uid = uid, uid.characters.count == 0 else { return nil }
        let option = LPKInfoFetchOption()
        option.message = message
        return LPKKit.shared.info(byUser: uid, option: option)?.showName
    }
    
    class func showNick(uid: String?, in session: NIMSession?) -> String? {
        guard let uid = uid, uid.characters.count != 0 else { return nil }
        let option = LPKInfoFetchOption()
    ...
    class func showTime(msglastTime: TimeInterval, showDetail: Bool) -> String {
        // 今天的时间
        let nowDate = Date()
        let msgDate = Date(timeIntervalSince1970: msglastTime)
        
        let components: Set<Calendar.Component> = [.year, .month, .day, .weekday, .hour]
        ...
        let minute = msgDateComponents.minute ?? 0
        let OnedayTimeIntervalValue = 24 * 60 * 60  //一天的秒数
        
        var result = LPKUtil.getPeriod(of: hour, with: minute)
        ...
        if nowDateComponents.day == msgDateComponents.day {
            result = String(format: "%@ %zd:%02d", result, hour, minute)
        }
        //昨天
        else if nowDateComponents.day == (msgDateComponents.day ?? 0 + 1) {
            result = showDetail ? String(format: "昨天%@ %zd:%02d", result, hour, minute) : "昨天"
        }
        //前天
        else if nowDateComponents.day == (msgDateComponents.day ?? 0 + 2) {
            result = showDetail ? String(format: "前天%@ %zd:%02d", result, hour, minute) : "前天"
        }
        //一周内
        else if nowDate.timeIntervalSince(msgDate) < TimeInterval(7 * OnedayTimeIntervalValue) {
            let weekDay = LPKUtil.weekdayStr(msgDateComponents.weekday ?? 0)
            result = showDetail ? weekDay.appendingFormat("%@ %zd:%02d", result, hour, minute) : weekDay
        }
        //显示日期
        else {
            let day = String(format: "%zd-%zd-%zd", msgDateComponents.year ?? 0, msgDateComponents.month ?? 0, msgDateComponents.day ?? 0)
            result = showDetail ? day.appendingFormat("%@ %zd:%02d", result,hour, minute) : day
        }
        return result
    }
    
    class func messageTipContent(in message: NIMMessage) -> String {
//        var text: String? = nil
//        switch message.messageType {
//        case .notification:
//            text = LPKUtil.showNick(uid: <#T##String?#>, in: <#T##NIMMessage?#>)
//        default:
//            <#code#>
//        }
        //
    ...
    class func canEditTeamInfo(_ member: NIMTeamMember) -> Bool {
    ...
    class func canInviteMember(_ member: NIMTeamMember) -> Bool {
        //    NIMTeam *team = [[NIMSDK sharedSDK].teamManager teamById:member.teamId];
...
extension LPKUtil {
    ...
    class func getPeriod(of time: Int, with minute: Int) -> String {
...
//    class func notificationMessage(_ message: NIMMessage) -> String {
